...
import { Users, Search, CheckCircle2, UserCheck, RefreshCw } from "lucide-react";
import { db } from "@/lib/firebase";
import { collection, onSnapshot } from "firebase/firestore";
import { loadIDBUsers } from "@/lib/contentService";
...
export function AdminGoogleUsersTab() {
  ...
  useEffect(() => {
    ...
    // Load durable IndexedDB users (restores accounts if LocalStorage & Cookies were cleared)
    ...
    const collectionsToListen = ["google_registered_users", "users", "registered_users", "students"];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredUsers.map((user) => (
            <div
              ...
              className="bg-surface-container border border-outline-variant/30 hover:border-primary/50 rounded-2xl p-5 flex items-center gap-4 transition-all hover:shadow-lg"
            >
              {user.photoURL ? (
                <img
                  src={user.photoURL}
